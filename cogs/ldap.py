...
class LDAP(commands.Cog):
    ...
    async def on_member_join(self, member):
        ...
        await member.add_roles(discord.utils.get(guild.roles, id = int(self.config['unverified_role'])), reason = None, atomic = True)
    ...
    async def on_raw_reaction_add(self, payload):
        ...
        if channel.id == int(self.config['auth_channel']) and user != self.bot.user:
            ...
            if message.embeds[0].title == 'AUTH REQUEST':
                if emoji == '\U0001f44d':
                    ...
                    await accepted_member.add_roles(discord.utils.get(guild.roles, id = int(self.config["verified_role"])), reason = None, atomic = True)
                    await accepted_member.remove_roles(discord.utils.get(guild.roles, id = int(self.config["unverified_role"])), reason = None, atomic = True)
                ...
                if emoji == '\U0001f44d':
                    ...
                    self.add_user(user = f'{first_last}', first_name = str(embed_fields[1].value), last_name = str(embed_fields[2].value), email = str(embed_fields[0].value))
        ...
        if channel.id == int(self.config['verification_channel']) and user != self.bot.user:
            ...
            if emoji == '\U0001f44d':
                ...
                auth_req_embed = discord.Embed(title=f'AUTH REQUEST')
                ...
                sent_embed_id = await awaiting_verification_channel.fetch_message(sent_embed.id)
    ...
    async def add_user_command(self, ctx, user: str, first_name: str, last_name: str, email: str, **kwargs):
        ...
        if user not in self.ldap.user_find(o_uid = user):
            new_user = self.add_user(a_uid = user, o_sn = last_name, o_givenname = first_name, o_cn = f'{first_name}  {last_name}', o_random = True)
            print(new_user)
        else:
    ...
    def add_user(self, user: str, first_name: str, last_name: str, email: str, **kwargs):
        if user not in self.ldap.user_find(o_uid = user):
            try:
                new_user = self.ldap.user_add(a_uid = user, o_sn = last_name, o_givenname = first_name, o_cn = f'{first_name}  {last_name}', o_mail = email, o_random = True)
                print(new_user)
    ...
    def _get_unverified_usernames(self):
        guild = self.guilds[0]
        #usernames = discord.utils.get(guild.roles)
